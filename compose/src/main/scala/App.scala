...
@main def main = OpenGL.Application(Title := state.map(_.toString())) {
  ...
  val title = Rx
    .usePeriod(1000, LocalDateTime.now(), { _ => LocalDateTime.now() })
    .map(_.format(DateTimeFormatter.ofPattern("yyyy年MM月dd日HH:mm:ss")))
  val (_123, set123) = useState(true)
  If(_123) {
    Window("123")(Title := title, DefaultWidth := 800, DefaultHeight := 800) {
      println("window123 called")
      Menu(Path += "123") {
        Action := { () => }
      }
  ...
  // Window("ALICE")(Title := title, DefaultWidth := 800, DefaultHeight := 800) {
  //   Menu(Path := Seq("123")) {
  //     Action := { () => }
  //   }
  //   OnKeyRelease(KeyCode := 256, Callback := { () => setBool(!bool.value) }) {}
