...
import com.github.yjgbg.compose.DocumentDsl.Layout.Style.Width
...
import com.github.yjgbg.compose.DocumentDsl.Layout.Style.DefaultPosition
...
object DocumentDsl extends DocumentDsl:
  def Application(closure0:Scope[Application] ?=> Unit)(closure1:Scope[Application] ?=> Unit) =
    ...
    rxObj.addListener{(oldApp,newApp) => setState(render(state.value,oldApp,newApp))}
    loop(state)
  ...
  def initAndRender:(Application => Map[String,Long],(Map[String,Long],Application,Application) => Map[String,Long]) = 
    ...
    (
      {app => 
        ...
        app(Window).map{(win) => win(Id) -> processWindow(Modify.CREATE(win),None).get}.toMap
      },
      {(state,oldApp,newApp)=> 
        val nextWinSeq = newApp(Window)
        val currentWinSeq = oldApp(Window)
        // id重合的减去对象本身重合的
        ...
        ++ needDelete.map{(bizId) => Modify.DELETE(nextWinSeq.find(_(Id)==bizId).get) -> Some(state(bizId))}
  ...
  def loop(state:Rx[Map[String,Long]]) = 
    import org.lwjgl
    import org.lwjgl.glfw
  ...
  def processWindow(action:Modify[Window],openglId:Option[Long]):Option[Long] = 
    import org.lwjgl.glfw
    import org.lwjgl
    action match
      ...
      case Modify.UPDATE(current, next) => 
        if (current(Name) != next(Name)) then glfw.GLFW.glfwSetWindowTitle(openglId.get,next(Name))
        openglId
      case Modify.CREATE(value) =>
        ...
        // 配置回调函数
        if value(OnExit) != null then glfw.GLFW.glfwSetWindowCloseCallback(winId,{(_) => value(OnExit).apply()})
