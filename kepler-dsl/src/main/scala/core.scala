...
object core:
  ...
  extension [S <:Scope](a:S) 
    ...
    def getSingleNode[Key <: String & Singleton,V](key:Key)(using SingleNodeKey[Key,S,V]):Option[S >> V] = 
      a.value.get(key).asInstanceOf
    def getMultiNode[Key <: String & Singleton,V](key:Key)(using MultiNodeKey[Key,S,V]):Seq[S >> V] = 
  ...
  object Key:
    ...
    inline def singleNodeKey[Key <: String & Singleton,S <: Scope,V]:SingleNodeKey[Key,S,V] = compiletime.constValue[Key].asInstanceOf
    inline def multiNodeKey[Key <: String & Singleton,S <: Scope,V]:MultiNodeKey[Key,S,V] = compiletime.constValue[Key].asInstanceOf
  import Scope.*
  ...
  extension [S <: Scope, K <: Singleton & String, V](key: K)
    ...
    def ++=(using MultiValueKey[K,S,V],S)(value:Iterable[V]):Unit =
      summon[S].value.get(key) match
        ...
        case Some(seq) => seq.asInstanceOf[collection.mutable.Buffer[V]] ++= value
    def apply(using either:Either[SingleNodeKey[K,S,V],MultiNodeKey[K,S,V]],s:S)(closure0: S >> V ?=> Unit)(closure1: S >> V ?=> Unit) :Unit =
      val x:S >> V = Scope.>>[S,V](collection.mutable.HashMap.empty)
      closure0(using x)
      closure1(using x)
      either match
        case Left(value) => s.value.put(key,x)
        case Right(value) => s.value.get(key) match
